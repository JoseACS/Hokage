...
"""
Hokage IRC Bot
Copyright 2016 - 2018, Jose A. Cantoral <js902402@gmail.com>
"""
import sys
import os
import optparse
...
def check_python_version():
    if sys.version_info < (2, 7):
        stderr(u'Error: Usted Necesita Tener Python 2.7!')
        sys.exit(1)
...
def main(argv=None):
    ...
    try:
        ...
        if not os.path.isfile(configpath):
            print u"Welcome to hokage's configuration! -- Bienvenido a la configuración de hokage!\n"
        ...
        if config_module.core.not_configured:
            stderr(u'The bot is not configured. -- El bot no está configurado.')
        ...
        if os.path.isfile(pid_file_path):
            ...
            if tools.check_pid(old_pid):
                if opts.quit is None and opts.kill is None:
                    stderr(u'There is already a hokage running. -- Ya hay un bot ejecutandose.')
                    stderr(u'Try -- Intenta: --quit o --kill')
                    sys.exit(1)
                elif opts.kill:
                    stderr(u'Killing bot. -- Matando al Bot.')
                    os.kill(old_pid, signal.SIGKILL)
                    sys.exit(0)
                elif opts.quit:
                    stderr(u'Quitting hokage. -- Desconectando a hokage.')
                ...
                stderr(u'The bot is not running. -- El bot no se esta ejecutando.')
                sys.exit(1)
